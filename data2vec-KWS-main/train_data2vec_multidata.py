...
from data2vec.data2vec_utils.trainer_multidata import train, evaluate
from utils.dataset_multidata import get_multidata_loader
...
def training_pipeline(config):
    ...
    # data
    with open(config["train_student_list_file"], "r") as f:
        train_student_list = f.read().rstrip().split("\n")
    
    with open(config["train_teacher_list_file"], "r") as f:
        train_teacher_list = f.read().rstrip().split("\n")

    with open(config["val_list_file"], "r") as f:
        val_list = f.read().rstrip().split("\n")

    trainloader = get_multidata_loader(train_student_list, train_teacher_list, config, train=True)
    ...
    # evaluating the final state (last.pth)conf
...
if __name__ == "__main__":
    ...
    # conf = "/home/jacobuni/uni-projects/p9-ssl_for_noise-robust_kws/data2vec-KWS-main/data2vec/data2vec_configs/multidata_config/kwt1_data2vec_config.yaml"
