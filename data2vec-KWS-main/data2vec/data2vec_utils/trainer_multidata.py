...
def train_single_batch(net: Data2Vec, data: torch.Tensor, mask: torch.Tensor, 
                       optimizer: optim.Optimizer, criterion: Callable, device: torch.device) -> Tuple[Any, Any, Any]:
    """
    ...
    data_student = data[0].to(device)
    data_teacher = data[1].to(device)

    optimizer.zero_grad()
    predictions, targets = net(data_student, data_teacher, mask)
...
def train(net: nn.Module, mask_generator, optimizer: optim.Optimizer, criterion: Callable, train_loader: DataLoader, 
    ...
    for epoch in range(config["hparams"]["n_epochs"]):
        ...
        for batch_index, data, in enumerate(train_loader):
            batch_size = data[0].size(dim=0)
            audio_length = data[0].size(dim=-1)
